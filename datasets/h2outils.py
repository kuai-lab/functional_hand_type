    ...
    for split_tag in frame_segments.keys():
        with open(os.path.join(path_dataset,'./action_labels/action_{:s}.txt'.format(split_tag)),'r') as f:
            ...
            for cline in segs:
                cline=cline.strip('\n').split(' ')
                if split_tag=='test':
                    seg_idx,action_idx=int(cline[0]), 2#int(cline[-1]) # WJ - given none action in test
                    start_frame_idx,end_frame_idx=int(cline[2]),int(cline[3])
                    action_name="grab chips"
                else:
...
def get_all_hand_labels():
    h2o_list_left, h2o_list_right = [], []
    for idx in range(1,5):
        h2o_list_left.append(pd.read_csv(f'./datasets/h2o_hand_label/left/subject{idx}.csv'))
        h2o_list_right.append(pd.read_csv(f'./datasets/h2o_hand_label/right/subject{idx}.csv'))
    
    both_list = list()
    for h2o_list in [h2o_list_left, h2o_list_right]:
        root_dict = dict()
        for file_idx, h2o in enumerate(h2o_list):
            root_dict[f"subject{file_idx + 1}"] = dict()

            for idx, key in enumerate(h2o[h2o.keys()[0]]):
                root_dict[f"subject{file_idx + 1}"][key] = dict()

                for video_idx in h2o.keys()[1:]:
                    try: data_list = h2o[video_idx][idx].strip().split(",")
                    except: continue

                    root_dict[f"subject{file_idx + 1}"][key][f"{video_idx}"] = []
                    for data in data_list: root_dict[f"subject{file_idx + 1}"][key][f"{video_idx}"].append(data)
        both_list.append(root_dict)
    return both_list

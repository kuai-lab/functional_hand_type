...
def get_all_hand_labels():
    ...
    for idx in range(1,7):
        cur_dir = os.getcwd()
        fphab_list.append(pd.read_csv(cur_dir + f'/datasets/fphab_hand_label/subject{idx}_fphab.csv'))

    root_dict = dict()
    for file_idx, fphab in enumerate(fphab_list):
        root_dict[f"Subject_{file_idx + 1}"] = dict()

        for idx, key in enumerate(fphab[fphab.keys()[0]]):
            root_dict[f"Subject_{file_idx + 1}"][key] = dict()

            for video_idx in fphab.keys()[1:]:
                try: data_list = fphab[video_idx][idx].strip().split(",")
                except: continue

                root_dict[f"Subject_{file_idx + 1}"][key][f"{video_idx}"] = []
                for data in data_list: root_dict[f"Subject_{file_idx + 1}"][key][f"{video_idx}"].append(data)
    return root_dict
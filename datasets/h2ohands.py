...
class H2OHands(object):
    def __init__(
        ...
        hand_type_info = h2outils.get_hand_type_info(path_hand_type_info)
        self.hand_type_info = hand_type_info

        self.num_handtypes = len(self.hand_type_info.keys())
        self.hand_labels = h2outils.get_all_hand_labels()
        self.h2o_hand_map = {-1:0, 1:1, 3:2, 4:3, 8:4, 9:5, 12:6, 14:7, 15:8, 17:9, 18:10, 20:11, 22:12, 23:13, 25:14, 27:15, 29:16, 33:17, 34:18, 35:0, 36:19, 37:20, 38:21, 39:22, 40:23}
    ...
    def get_hand_label(self, idx):
        ...
        subject = path_info[0]
        scene = path_info[1]
        sequence = path_info[2]
        frame_number = int(path_info[-1].split('.')[0])
        try:
            left_frame_labels = self.hand_labels[0][subject][scene][sequence]
            right_frame_labels = self.hand_labels[1][subject][scene][sequence]
        except KeyError:
            print('[KeyError] img path ===>', img_path)
        
        both_labels = list()

        for frame_range in left_frame_labels:
            ...
            if start_frame <= frame_number and frame_number <= last_frame:
                try:
                    ...
                    hand_label = -1
                both_labels.append(self.h2o_hand_map[hand_label])
        
        if len(both_labels) == 0:
            both_labels.append(0)

        for frame_range in right_frame_labels:
            range_label = frame_range.split(':')
            try:
                start_frame = int(range_label[0].split('-')[0])
                last_frame = int(range_label[0].split('-')[1])

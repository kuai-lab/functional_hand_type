...
class FHBHands(object):
    def __init__(
        ...
        path_hand_type_info = os.path.join(self.root, 'hand_type_info.txt')
        hand_type_info = fhbutils.get_hand_type_info(path_hand_type_info)
        self.hand_type_info = hand_type_info

        self.num_handtypes = len(self.hand_type_info.keys())
        self.hand_labels = fhbutils.get_all_hand_labels()
        self.fpha_hand_map = {-1:0, 1:1, 3:2, 4:3, 8:4, 9:5, 12:6, 14:7, 15:8, 17:9, 18:10, 20:11, 22:12, 23:13, 25:14, 27:15, 29:16, 33:17, 34:18, 36:19, 37:20, 38:21, 39:22, 40:23}
    ...
    def get_hand_label(self, idx):
        idx = self.get_dataidx(idx)
        img_path = self.image_names[idx]
        path_info = img_path.split('/')
        subject = path_info[0]
        action_label = path_info[1]
        video_idx = path_info[2]
        frame_number = int(path_info[-1].split('_')[1].split('.')[0])
        try:
            frame_labels = self.hand_labels[subject][action_label][video_idx]
        except KeyError:
            print('[KeyError] img path ===>', img_path)
        
        for frame_range in frame_labels:
            ...
            if start_frame <= frame_number and frame_number <= last_frame:
                try:
                    ...
                    print('[ValueError] frame labels, frame number, img path ===> ', frame_labels, frame_number, img_path)
                    hand_label = -1
                return self.fpha_hand_map[hand_label]
        print('[NoneError] img path ===> ', img_path)

...
def epoch_pass(
    ...
    for batch_idx, batch in enumerate(tqdm(loader)): 
        ...
        if not train:
            ...
            # WJ
            batch_subject.extend([i for i in batch["sample_info"]["subject"][::ntokens]])
            batch_scene.extend([i for i in batch["sample_info"]["scene"][::ntokens]])
            batch_sequence.extend([i for i in batch["sample_info"]["sequence"][::ntokens]])
            result_list.extend([results["action_pred_labels"][0].item(), results["action_pred_labels"][1].item()])
            result_pos_list.extend(results["action_reg_possibilities"].detach().cpu())
    ...
    import os
    import json
    #with open(os.path.join('../HTT_old/h2o/data/action_labels/action_test.txt'),'r') as f:
    with open(os.path.join('/home/shu/Desktop/data/action_labels/action_test.txt'),'r') as f:
        segs=f.readlines()[1:] 
        skip_num = 0
        for idx, cline in enumerate(segs):
            cline=cline.strip('\n').split(' ')
            frame_nums = int(cline[3]) - int(cline[2])
            real_result.append(result_list[idx+skip_num]+1)
            skip_num += frame_nums // (128*2)
    
    data_dict = dict()
    data_dict["modality"] = {"train":"RGB+hand", "test":"RGB"}
    for idx, value in enumerate(real_result): data_dict[str(idx+1)] = value

    with open('action_labels.json', 'w') as f: json.dump(data_dict, f, indent=2)
    print(real_result, len(real_result))
